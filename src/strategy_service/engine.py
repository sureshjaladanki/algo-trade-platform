...
class StrategyEngine:
    ...
    def _init_trade_symbol_data(self, symbol: str):
        ...
        symbol_1d_candles_history_dataframe = self.data_adapter.read_1d_candles_history_dataframe(symbol)
        day_atr_profile = get_atr(symbol_1d_candles_history_dataframe)

        # symbol 1min candles history dataframe
        symbol_1m_candles_history_dataframe = self.data_adapter.read_1m_candles_history_dataframe(symbol)
        minute_of_day_volume_profile = get_volume_profile(symbol_1m_candles_history_dataframe)
        symbol_profile = {
        ...
        symbol_1min_candles_prev_day_dataframe = symbol_1m_candles_history_dataframe.loc[str(latest_date.date())].copy()
        self.symbol_dataframes[symbol] = symbol_1min_candles_prev_day_dataframe
    
        if self.trade_symbols[symbol].get('can_long', False):
            self.long_strategies[symbol] = LongStrategy(symbol, symbol_profile=symbol_profile, regime=self.regime_strategy)
        if self.trade_symbols[symbol].get('can_short', False):
            self.short_strategies[symbol] = ShortStrategy(symbol, symbol_profile=symbol_profile, regime=self.regime_strategy)
    ...
    def _on_1m_candle_closed(self, symbol: str, candle: pd.Series):
        ...
        if symbol in self.symbol_dataframes: 
            ...
            if self.symbol_dataframes[symbol].empty and len(self.symbol_dataframes[symbol].columns) == 0:
    ...
    def _generate_regime_features(self):
        ...
        self.regime_strategy.generate_features(self.symbol_dataframes[self.regime_symbol], { symbol: self.symbol_dataframes[symbol] for symbol in self.advance_decline_symbols })
    ...
    def _generate_trade_features(self, symbol: str):
        ...
        if symbol in self.long_strategies:
            self.long_strategies[symbol].generate_features(self.symbol_dataframes[symbol])
        elif symbol in self.short_strategies:
            self.short_strategies[symbol].generate_features(self.symbol_dataframes[symbol])
    ...
    def run(self):
        ...
        all_signals = []
        regime_generator = self.data_adapter.process_next_1m_candle(self.config.get("regime_symbol", None))
