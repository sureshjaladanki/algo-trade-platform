...
def get_atr(df: pd.DataFrame, period: int = 14) -> dict:
    ...
    if atr_series is not None and isinstance(atr_series, pd.Series) and not atr_series.empty:
        # Get the latest non-NaN value
        latest_atr = atr_series.dropna().iloc[-1] if not atr_series.dropna().empty else None
        latest_close = df['close'].iloc[-1]

        if latest_atr is not None:
            return {
                'prev_atr': float(latest_atr),
