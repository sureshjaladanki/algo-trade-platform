...
class RegimeStrategy:
    ...
    def __init__(self):
        ...
        self.vix_levels = self.config.get('vix_levels', {'low': 15.0, 'high': 20.0, 'panic': 25.0})
    ...
    def generate_features(self, vixdf: pd.DataFrame, component_dfs: Dict[str, pd.DataFrame]):
        ...
        if vixdf is not None and not vixdf.empty:
            ...
            vixdf = add_ad_ratio(vixdf, component_dfs)
    ...
    def safe_for_longs(self) -> bool:
        ...
        return self.data["vix"] < self.vix_levels["panic"] and self.data["ad_ratio"] > self.ad_ratio_thresholds["bearish"]
