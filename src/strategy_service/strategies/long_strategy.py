...
class LongStrategy(TradeStrategy):
    ...
    def check_entry(self, data):
        ...
        macro_trend_up = data.get(self.ema_fast_key, 0) > data.get(self.ema_slow_key, 0)
        ...
        return safe_for_intraday_positions and safe_for_long and macro_trend_up and macro_trend_strong and micro_trend_up and pullback and sufficient_volatility and sufficient_margin and favorable_rr
    ...
    def exit(self, data, prev_data=None):
        ...
        current_price_below_9ema = data['close'] < data.get(self.ema_micro_fast_key, 0)
        prev_price_above_9ema = prev_data['close'] >= prev_data.get(self.ema_micro_fast_key, 0)
        take_profit = rsi_overbought and current_price_below_9ema and prev_price_above_9ema
        
        # Exit 3: Macro Trend Reversal (Simulated 5m 9 EMA crosses below 21 EMA)
        current_trend_down = data.get(self.ema_fast_key, 0) < data.get(self.ema_slow_key, 0)
        prev_trend_up = prev_data.get(self.ema_fast_key, 0) >= prev_data.get(self.ema_slow_key, 0)
        ...
        volatility_exhaustion = bb_hit_upper_band and current_price_below_9ema and prev_price_above_9ema
