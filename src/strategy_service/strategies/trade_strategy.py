...
class TradeStrategy(ABC):
    ...
    def generate_features(self, df):
        ...
        if self.symbol_profile is not None:
            if 'minute_of_day_volume' in self.symbol_profile:
                avg_volume = df['minute_of_day'].map(self.symbol_profile['minute_of_day_volume'])
                df['rvol'] = df['volume'] / avg_volume
            else:
                df['rvol'] = 1
            
            if all(key in self.symbol_profile for key in ['prev_close', 'prev_atr']) and self.gap_atr_ratio is None:
    ...
    def check_entry(self, data):
        ...
        if self.regime:
            safe_for_intraday_positions = TradingSession.WARMUP < self.regime.trading_session < TradingSession.CLOSING
        else:
        ...
        session_name = self.regime.trading_session.name.lower() if self.regime else 'opening'
        thresholds = self.config.get('session_volume_threshold', {})
        threshold = thresholds.get(session_name, 1.0) if isinstance(thresholds, dict) else 1.0
        ...
        gap_atr_ratio_within_limit = abs(self.gap_atr_ratio) <= gap_atr_ratio_limit if self.gap_atr_ratio is not None else True

        return safe_for_intraday_positions and trading_volume_above_threshold and gap_atr_ratio_within_limit

    def check_exit(self, data, prev_data=None):
        """Determine if exit needs to be mandated."""
        current_exit = self.regime.trading_session > TradingSession.CLOSING if self.regime else False
