...
def fetch_yfinance_ticks(symbol: str, interval: str, period: str):
    ...
    if df.empty:
        print(
            f"Error: No data found for {symbol} with interval='{interval}' and period='{period}'."
        )
        print(
            "Please check if the symbol is correct and if the combination of interval and period is supported by Yahoo Finance."
        )
        sys.exit(1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pull historical tick/candle data from yfinance."
    )
    parser.add_argument(
        "--symbol",
        type=str,
        required=True,
        help="Ticker symbol (e.g., AAPL, MSFT, ^NSEI)",
    )
    parser.add_argument(
        "--interval",
        type=str,
        required=True,
        help="Tick interval (e.g., 1m, 5m, 1h, 1d)",
    )
    parser.add_argument(
        "--period",
        type=str,
        required=True,
        help="Data period (e.g., 1d, 5d, 1mo, 1y, max)",
    )
