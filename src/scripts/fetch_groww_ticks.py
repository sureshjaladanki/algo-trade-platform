...
def map_interval_to_groww_format(interval: str) -> str:
    ...
    if interval not in mapping:
        raise ValueError(
            f"Unsupported interval format: {interval}. Supported: {list(mapping.keys())}"
        )
...
def fetch_groww_ticks(symbol: str, interval: str, period: str):
    ...
    print(
        f"Fetching data from Groww API for {symbol} | Interval: {interval} | Period: {period}..."
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Pull historical tick/candle data from Groww Official API."
    )
    parser.add_argument(
        "--symbol",
        type=str,
        required=True,
        help="Ticker symbol (e.g., RELIANCE, NIFTYBEES)",
    )
    parser.add_argument(
        "--interval",
        type=str,
    ...
    parser.add_argument(
        "--period", type=str, required=True, help="Data period (e.g., 1d, 7d, 1mo, 1y)"
    )
