...
from src.horizon.eval.path_density import evaluate_path_density
...
def _run_fold(
    ...
    print("Loading Horizon universe + building features/labels/TB+MFE...")
    ...
    metrics = evaluate_path_density(
        scored, directions=scored_dirs, n_boot=n_boot, seed=seed
    )
    title = (
        f"Path-density Step 0  fold={fold}  train={train_period}  "
        f"test={test_period}  dirs={','.join(scored_dirs)}  (REPORT ONLY -- no peek)"
    )
    print()
    # Force UTF-8 so en-dash / minus glyphs in MetricResult notes do not crash cp1252.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Path-density Step 0: Top-K vs Rest MFE + TB exit mix on Fold A/B. "
            "Not a peek — hard-stop input before any Long lever."
        )
    )
    ...
    print("Path-density Step 0 -- REPORT ONLY (no peek)")
    ...
    # Cross-fold hard-stop summary (Long primary; Short diagnostic).
    print("\n=== Step 0 hard-stop summary ===")
    for direction in directions:
        seps = [
            m
            for m in all_metrics
            if m.name == "SEP" and m.side == direction
        ]
        if not seps:
            print(f"  {direction}: no SEP metrics")
            continue
        any_sep = any(m.gate_pass for m in seps)
        all_sep = all(m.gate_pass for m in seps)
        print(
            f"  {direction}: fold_SEP={[bool(m.gate_pass) for m in seps]} "
            f"any={any_sep} both={all_sep} "
            f"-> {'DENSITY SIGNAL' if any_sep else 'NO SEPARATION (STOP candidate)'}"
        )
