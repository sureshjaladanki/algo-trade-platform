...
def _run_fold(
    ...
    print(
        f"Fitting locked Long path-EV; Peek 1 P(SL) veto P{int(quantile * 100)} "
    ...
    if veto_model is None:
        print(f"Error: veto fit failed: {full_stats}")
        sys.exit(1)
    veto_features = list(full_stats["features"])
    print(f"   Veto model train_bars={full_stats.get('train_bars')}")

    metrics = evaluate_psl_veto_peek(
        scored,
    ...
    title = (
        f"Path-quality veto Peek 1  fold={fold}  "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Path-quality veto Peek 1: inference-only P(SL) worst-quantile cut. "
            "Requires Step 0 H5 + null-lever + min-power clear."
        )
    )
    ...
    parser.add_argument("--quantile", type=float, default=DEFAULT_VETO_QUANTILE)
    parser.add_argument("--a2-min-bars", type=int, required=True)
    parser.add_argument("--a2-min-sessions", type=int, required=True)
    ...
    print(
        f"Path-quality veto Peek 1 -- P(SL) > elig P{int(args.quantile * 100)} "
    ...
    for name in ("H5", "H1", "H2", "H3", "A1", "A2", "NULLnarrow", "TBadm"):
        ms = [m for m in all_metrics if m.name == name and m.side == "long"]
        if not ms:
            continue
        gates = [m.gate_pass for m in ms]
        vals = [f"{m.value:.4f}" if m.value is not None else "-" for m in ms]
