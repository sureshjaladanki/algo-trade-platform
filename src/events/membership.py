...
The replacement ledger is the in-repo membership source (cascade orphan).
Daily flags are walked from the Dec-2025 constituent set. Dates on the
ledger are effective dates; announcement dates are not recoverable from it.
"""
...
# Canonical `.NS` tickers as of 8 Dec 2025.
...
REPLACEMENTS: tuple[tuple[dt.date, str, str], ...] = (
...
# GOLDEN names that refer to the same listing as a canonical member.
ALIASES: dict[str, str] = {
    "TATAMOTORS.NS": "TMPV.NS",
    "MCDOWELL-N.NS": "UNITDSPR.NS",
}

ANNOUNCEMENT_DATE_STATUS = "unrecoverable_from_pit"
...
def nifty50_members_on(as_of: dt.date) -> frozenset[str]:
    ...
    for event_date, excluded, included in reversed(REPLACEMENTS):
        if event_date > as_of:
...
def membership_pairs(dates: list[dt.date]) -> pl.DataFrame:
    """Long (date, symbol) table including GOLDEN aliases."""
    rows: list[dict] = []
    ...
    for day in dates:
        for symbol in nifty50_members_on(day):
            key = (day, symbol)
            if key not in seen:
                rows.append({"date": day, "symbol": symbol})
                seen.add(key)
            for alias in reverse_alias.get(symbol, ()):
                alias_key = (day, alias)
                if alias_key not in seen:
                    rows.append({"date": day, "symbol": alias})
                    seen.add(alias_key)
    return pl.DataFrame(rows)


def covered_families() -> tuple[str, ...]:
    """Index families with PIT membership in this repository."""
    return (FAMILY_NIFTY_50,)
