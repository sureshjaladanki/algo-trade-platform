...
GL0 adds LSE session-window refusals on the same module. No third run loop.
Precedence: N6 receipt → N5 situs → GL-H3 cliff margin → session window.
Receipt and situs have no override. The cliff calendar outranks the window.
"""
...
from src.receipt import assert_us_cash_leg
...
from src.situs import assert_us_situs_cap, refuse_uk_register
...
class SessionWindowRefused(Exception):
    """Order is outside the overlap window and no reasoned override was logged."""


class CliffMarginRefused(Exception):
    """Trade date is after the latest safe date, or the receipt straddles the cliff."""
...
class Order:
    ...
    uk_register: bool
    us_situs_book_usd: float
    us_situs_additional_usd: float = 0.0
    override: str = ""
    override_reason: str = ""
...
def instructions_from_schedule(result, auth: AuthState, phase: SessionPhase, *, order_type: OrderType = OrderType.LIMIT):
...
def lots_after(lots: list, instruction: Instruction, price: Decimal) -> list:
...
def place(
    ...
    *,
    ...
    from src.portfolio import LimitError, assert_portfolio
...
def screen(order: Order) -> ScreenResult:
    assert_us_cash_leg(order.settlement_country)
    refuse_uk_register(order.uk_register)
    assert_us_situs_cap(
        book_usd=order.us_situs_book_usd,
        additional_usd=order.us_situs_additional_usd,
    )

    cliff = rnor_cliff()
    if straddles_cliff(order.trade_date, order.venue, cliff):
        raise CliffMarginRefused(
            f"receipt for {order.trade_date} on {order.venue} straddles {cliff}"
        )

    latest_safe = latest_safe_trade_date(cliff)
    if order.trade_date > latest_safe:
        raise CliffMarginRefused(
            f"{order.trade_date} is after latest safe trade date {latest_safe}"
        )

    if in_session_window(order.exposure_class, order.trade_date, order.london_time):
        return ScreenResult(allowed=True, override="", reason="inside session window")

    if (
        order.trade_date == latest_safe
        and order.override == "cliff_margin"
        and order.override_reason
    ):
        LOGGER.info(
            "cliff-margin override: window waived on %s at %s (%s)",
            order.trade_date,
            order.london_time,
    ...
    if order.override == "session_window" and order.override_reason:
        LOGGER.info(
            "session-window override: %s at %s (%s)",
            order.trade_date,
            order.london_time,
            order.override_reason,
        )
        return ScreenResult(
            allowed=True,
            override="session_window",
            reason=order.override_reason,
        )

    raise SessionWindowRefused(
        f"{order.exposure_class} at {order.london_time} London is outside the window"
    )
