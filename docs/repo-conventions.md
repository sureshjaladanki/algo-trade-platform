...
poetry run python -m src.events.constants
poetry run pytest
...
Add a runtime package only when a milestone needs it. v1 does not include HMM, LightGBM, MLflow, or Kaggle clients.
...
| Path | Role |
|------|------|
| `src/events/` | Event pool, ranking rules, cost and tax constants |
| `tests/` | Pytest |
...
New docs go under `docs/` with kebab-case names. Research stop-memos and closed-programme summaries belong in `docs/archive/`; active next work in `docs/next/`.
