...
Add a runtime package only when a milestone needs it. v1 does not include HMM, LightGBM, MLflow, or a Kaggle client. Polars is not a seed dependency; add it when a panel milestone imports it. Secrets (`KAGGLE_API_TOKEN` and others) live in gitignored `.env`; copy `.env.example`.
